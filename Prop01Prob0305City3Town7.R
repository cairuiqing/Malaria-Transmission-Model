...
#                pr_on_feed_rainy = 0.15, pr_on_feed_dry = 0.05, pr_on_feed_moderate = 0.1, pr_hum_to_mos = 0.6, pr_mos_to_hum = 0.3,
#                pr_num_biting = c(0.6, 0.35, 0.04, 0.01, 0, 0, 0), n_m=c(60000,30000,30000),proportion_suceptible = 0.2,pr_suceptibility=0.01,
#                pr_nonSuceptibility=0.005,
#                n_p= c(400,200,200),proportion_mobile = 0.1, pr_move = c(0.03,0.05,0.05), num_loc=3, n_days = 730,
#                scenario_name = "Prop01Prob030505LHHtest", n_sim=5, prob_matrix= matrix(c(NA, 0.3485742,0.6514258,
#                                                                                          0.2892406,NA,0.7107594,
#                                                                                          0.2467261,0.7532739,NA), nrow = 3))

n_p <- c(400, 400, 400, 200, 200, 200, 200, 200, 200, 200)
n_m = c(60000, 60000, 60000, 30000, 30000, 30000, 30000, 30000, 30000, 30000)
prob_matrix <- matrix(c(NA, 0.18927454, 0.2172186, 0.08067833, 0.05954918, 0.08407578, 0.08209966, 0.08445692, 0.10393687, 0.09871013, 
                        ...
                        0.5076943, 0.3310045, 0.1304243, 0.00781055, 0.0061867, 0.00572986, 0.00814565, 0.0088086, 0.01539554, NA), nrow = 10)

num_loc <- length(n_p)
run_biting_sim(pr_symp_infec = 0.05, pr_symp_non_infec = 0.05, pr_clear = 0.85, pr_off_feed = 0.01,
               pr_on_feed_rainy = 0.15, pr_on_feed_dry = 0.05, pr_on_feed_moderate = 0.1, pr_hum_to_mos = 0.6, pr_mos_to_hum = 0.3,
               pr_num_biting = c(0.6, 0.35, 0.04, 0.01, 0, 0, 0), n_m=n_m,proportion_suceptible = 0.2,pr_suceptibility=0.01,
               pr_nonSuceptibility=0.005,
               n_p= n_p,proportion_mobile = 0.1, pr_move = c(rep(0.03, 3), rep(0.05, 7)), num_loc=num_loc, n_days = 730,
               scenario_name = "Prop01Prob0305City3Town7", n_sim=5, prob_matrix=prob_matrix)
