...
#############################
track_transmission <- function(day, infec_p, infec_m, human_locs, haplotypes, location_matrix, travel_status) {
  transmission_matrix <- matrix(0, nrow = sum(n_p), ncol = num_loc)
  haplotype_log <- list()
  mobility_matrix <- matrix(0, nrow = num_loc, ncol = num_loc)
  
  for (i in 1:sum(n_p)) {
    if (length(infec_p[[i]]) > 0) {
      for (hap in infec_p[[i]]) {
        source_location <- human_locs[i]
        if (travel_status[i] == 1) {
          source_location <- location_matrix[i, day]
        }
        transmission_matrix[i, source_location] <- 1
        haplotype_log[[i]] <- c(haplotype_log[[i]], hap)
        mobility_matrix[source_location, human_locs[i]] <- 
          mobility_matrix[source_location, human_locs[i]] + 1
      }
    }
  }
  
  write.csv(transmission_matrix, paste0("Infection_Tracking_Data/transmission_day_", day, ".csv"))
  saveRDS(haplotype_log, file = paste0("Infection_Tracking_Data/haplotype_log_day_", day, ".rds"))
  write.csv(mobility_matrix, paste0("Infection_Tracking_Data/mobility_day_", day, ".csv"))
}
...
run_biting_sim<- function(pr_symp_infec, pr_symp_non_infec, pr_clear, pr_off_feed, pr_on_feed_rainy, pr_on_feed_dry, 
  ...
  for(q in 1:n_sim){
    ...
    for(r in 1:n_days){
      ...
      track_transmission(r, infec_p, infec_m, human_locs, haps, location, mobile_humans)
    }
