...
#####################################################
n_p<- c(20, 20, 20, 40, 40)#20, 20, 20, 20, 40, 40, 40)
n_m<- c(3000, 3000, 3000, 6000, 6000)#3000, 3000, 3000, 3000, 6000, 6000, 6000)

num_loc <- length(n_p)
prob_matrix <- matrix(runif(num_loc * num_loc), nrow = num_loc)
diag(prob_matrix) <- NA
...
for(q in 1:n_sim){
  ...
  for(r in 1:n_days){
    ...
    # Track human movement and haplotype transfer
    current_day <- r
    moved_humans <- which(humans_moving == 1)
    for (ip in moved_humans) {
      origin_loc <- init_locs_p[ip]
      dest_loc <- human_locs[ip]
      
      # effective haplotypes from source human
      valid_haps <- which(age_haps_p[ip, ] >= 7 & age_haps_p[ip, ] <= 90) # haplotypes that still in infectious period
      if(length(valid_haps) > 0){
        haplotype_export[[q]] <- rbind(haplotype_export[[q]],
                                       data.frame(
                                         day = current_day,
                                         human_id = ip,
                                         origin = origin_loc,
                                         haplotypes = paste(haps[valid_haps], collapse=","),
                                         destination = dest_loc
                                       ))
      }
    }
    
    # Tracking haplotype from human to mosquito
    if(length(new_haps) > 0){
      human_origin_loc <- init_locs_p[i] # human original location
      current_loc <- human_locs[i] # current location of human
      
      for(h in new_haps){
        haplotype_import[[q]] <- rbind(haplotype_import[[q]],
                                       data.frame(
                                         day = current_day,
                                         haplotype = h,
                                         new_loc = current_loc,
                                         mosquito_id = mos_index[j],
                                         infected_humans = NA  
                                       )
        )
      }
      
      # transmission chain for each mosquito
      mosquito_trans_chain <- vector("list", sum(n_m))
      mosquito_trans_chain[[mos_index[j]]] <- list(
        origin_loc = human_origin_loc,
        haplotype = new_haps,
    ...
    if(length(new_haps_m) > 0){
      trans_chain_info <- mosquito_trans_chain[[mos_index[j]]]
      # Tracking haplotype from mosquito to human
      for(h in new_haps_m){
        idx <- which(haplotype_import[[q]]$haplotype == h & 
                       haplotype_import[[q]]$mosquito_id == mos_index[j])
        idx <- na.omit(idx)
        
        if(length(idx) > 0){
          current_record <- haplotype_import[[q]]$infected_humans[idx]
          if (is.na(current_record) || current_record == "") {
            current_ids <- character(0)
          } else {
            current_ids <- unlist(strsplit(current_record, ","))
          }
          updated_ids <- unique(c(current_ids, as.character(i)))
          haplotype_import[[q]]$infected_humans[idx] <- ifelse(
            length(updated_ids) > 0,
            paste(updated_ids, collapse = ","),
            NA_character_
          )
        }
        
        origin_loc <- trans_chain_info$origin_loc
        dest_loc <- human_locs[i] 
        if(origin_loc != dest_loc){
          origin_dest_matrix[origin_loc, dest_loc, q] <- 
            origin_dest_matrix[origin_loc, dest_loc, q] + 1
        }
      }
    }
...
#   n_p = n_p, 
#   proportion_mobile = 0.1, 
#   pr_move = rep(0.03, num_loc), 
#   n_days = 730,
